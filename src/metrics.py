...
class DrowsinessMetrics:
    ...
    def __init__(
        ...
        self.perclos_window_seconds = float(
            perclos_window_seconds
        )

        self.microsleep_seconds = float(
            microsleep_seconds
            if microsleep_seconds is not None
            else self.MICROSLEEP_SECONDS
        )

        self.calibration_ratio = float(
            calibration_ratio
            if calibration_ratio is not None
            else self.CALIBRATION_RATIO
        )
        ...
        self.closed_eye_ratio = float(
            closed_eye_ratio
            if closed_eye_ratio is not None
            else self.CLOSED_EYE_RATIO
        )

        self.ear_threshold = self.fallback_threshold

        self.closed_eye_threshold = (
            self.ear_threshold * self.closed_eye_ratio
        )

        self.ear_history = deque(
            maxlen=max(
                1,
                int(self.fps * self.perclos_window_seconds)
            )
        )

        self.blink_history = deque(
            maxlen=max(1, int(self.fps * 60))
        )
        ...
        self._closure_min_frames = max(
            2,
            int(self.fps * 0.10)
        )

        self._microsleep_frames = max(
            1,
            int(self.fps * self.microsleep_seconds)
        )

        # Grace period: allow up to this many consecutive open frames
        # without resetting the microsleep counter.
        self._grace_frames = max(
            2,
            int(self.fps * 0.08)
        )
        ...
        self._cal_target = int(
            self.CALIBRATION_SECONDS * self.fps
        )

        self.calibrating = True
        self.baseline_ear: float | None = None

        # Per-eye asymmetry tracking
        self._left_ear_history: deque[float] = deque(
            maxlen=max(1, int(self.fps * 3))
        )

        self._right_ear_history: deque[float] = deque(
            maxlen=max(1, int(self.fps * 3))
        )
    ...
    def update_fps(self, fps: float) -> None:
        ...
        self._left_ear_history = deque(
            maxlen=max(1, int(self.fps * 3))
        )

        self._right_ear_history = deque(
            maxlen=max(1, int(self.fps * 3))
        )
    ...
    def _finish_calibration(self) -> None:
        ...
        if len(samples) < 10:
            ...
            print(
                "\nCalibration had too few samples — "
                "using fallback EAR threshold."
            )
            ...
            self.baseline_ear = float(
                np.median(open_samples)
            )

            self.ear_threshold = round(
                self.baseline_ear * self.calibration_ratio,
                4
            )

            self.closed_eye_threshold = round(
                self.ear_threshold * self.closed_eye_ratio,
                4
            )
            ...
            print(
                ...
                f"closed threshold: "
                f"{self.closed_eye_threshold:.3f}"
            )
    ...
    def recalibrate(self) -> None:
        ...
        print(
            "\nRecalibrating — "
            "look straight ahead for 30 seconds."
        )
    ...
    def get_ear_asymmetry(self) -> float:
        """Return mean |left_EAR - right_EAR| over the last 3 seconds."""

        if len(self._left_ear_history) < 5:
            return 0.0

        diffs = np.abs(
            np.array(self._left_ear_history)
            - np.array(self._right_ear_history)
        )

        return float(np.mean(diffs))

    def _update_blink_and_closure(
        self,
        ear: float
    ) -> None:
        ...
        if closed:
            ...
            if self._open_grace_count > self._grace_frames:
                ...
                # Genuinely open long enough —
                # finalise the previous closure.

                if (
                    self._min_blink_frames
                    <= self._closed_frames
                    < self._microsleep_frames
                ):
    ...
    def get_blink_rate(self) -> float | None:
        """Return blinks/min once 20 seconds of data is available."""
        ...
        while (
            self._blink_events
            and self._blink_events[0] < cutoff
        ):
            self._blink_events.popleft()

        duration_s = (
            min(self._frame_index, window_frames)
            / self.fps
        )
    ...
    def update(
        ...
        if not eyes_reliable:
            ...
            # Clamp to threshold so extreme head angles
            # don't spike PERCLOS.
        ...
        self._update_blink_and_closure(
            avg_ear
        )

        microsleep_dur = (
            self.get_microsleep_duration()
        )
